  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const savedOrders = localStorage.getItem('user_purchases');
        if (savedOrders) {
          setPurchasedProducts(JSON.parse(savedOrders));
        } else {
          const legacyOrder = localStorage.getItem('last_purchase');
          if (legacyOrder) {
            setPurchasedProducts([JSON.parse(legacyOrder)]);
          } else {
              ...
              >
                <div className="flex items-center gap-4 w-full sm:w-auto">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-black/50 block">Jumlah: {item.qty || 1}</span>
                    <p className="text-sm font-semibold text-[#059669] mt-0.5">{item.price}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:items-end gap-2 self-end sm:self-center">
                  ...
                  <button 
                    ...
                    className="text-sm bg-[#059669] py-2 px-3 rounded-md font-normal text-white hover:scale-103 active:scale-95 transition-all duration-200"
                  >
                    Detail Pesanan
                  </button>
                </div>
